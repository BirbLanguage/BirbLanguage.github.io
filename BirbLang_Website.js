...
function conditionals(){
	...
	text_stuff.innerHTML = "Conditionals in programming are statements which test whether a certain condition is true or false, and will return either value accordingly. For example, the conditional <i>5 > 2</i> would return a boolean value of True because the statement is, in fact, true.<br><br>As with most other languages, BirbLang can test a variety of different conditions.<br><b>x is floofier than y</b> will return true if the value of <i>x</i> is greater than the value of <i>y</i>.<br><b>x if floofier than or just as floofy as y</b> will return True if <i>x</i> is greater than or equal to the value of <i>y</i>.<br><b>x is not as floofy as y</b> will return True if the value of <i>x</i> is less than the value of <i>y</i> (or, for strings and lists, if the value of <i>x</i> is not equal to the value of <i>y</i>).<br><b>x is not as floofy or just as floofy as y</b> will return true if the value of <i>x</i> is less than or equal to the value of <i>y</i>.<br><b>x is just as floofy as y</b> will return True if the value of <i>x</i> is equal to the value of <i>y</i> (this works with strings and lists as well).<br><br><b>Example:</b><br><div class = 'code'>hatch egg<br>new birb egg is 5<br>squawk egg is floofier than 2<br>squawk 2 is floofier than egg<br>slep</div><div class = 'output'>Output:<br>True<br>False</div><br>Additionally, multiple conditions can be tested as one large condition by separating each condition with either <i>or</i> or <i>and</i>. Conditions separated with <i>or</i> will return True if 1 of out any of the conditions returns True, while conditions separated by <i>and</i> will only return True if all conditions evaluate to True as well (otherwise, it will return False).<br><br><b>Example:</b><br><div class = 'code'>hatch egg<br>new birb foo is 5<br>squawk foo is floofier than 2 or foo is not as floofy as 3<br>squawk foo is floofier than 2 and foo is not as floofy as 3<br>slep</div><div class = 'output'>Output:<br>True<br>False</div>";
	highlightAll();
}
